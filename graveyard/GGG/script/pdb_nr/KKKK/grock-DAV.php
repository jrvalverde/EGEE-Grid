...
$local_tmp_directory="/data/www/EMBnet/tmp/grid/david/new";

//User Interface directory
$UI_data_path="/tmp/grock/data/david/new";
...
function connect($UI_data_path)
{
    ...
    //fwrite($pipes[0], "grid-proxy-init -pwstdin -debug\n");
...
function grock($local_pdb_path, $UI_data_path, $local_tmp_directory, $host)
{
	...
	$descriptorspec=$conn_values[0];
	    ...
	    fwrite($pipes[0], "ls ; mkdir -p $UI_directory\n");
	    fflush($pipes[0]);
	    ...
	    exec("mkdir $local_directory");
    	    if ($debug)
	    	echo  date("dS of F h:i:s") . " [R] mkdir -p $UI_directory\n";
	    fwrite($pipes[0], "ls; /bin/mkdir -p $UI_directory\n");
	    fflush($pipes[0]);
...
//    fwrite($pipes[0], "edg-job-submit  -o $UI_directory/identifier.txt $UI_directory/grock.jdl > submit.txt &\n");
    if ($debug)
    	echo date("dS of F h:i:s") . " [R] grid-proxy-info > grid-proxy-info.txt &\n";
