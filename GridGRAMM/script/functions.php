 ...
 */
function enter_grid($pipes, $grid_pass)
{
...
{
// Take a shortcut
	exec("tar -zxf */gramm-come.tar.gz -C .");
	return $ret="OK";

// We search the directory with the user results!!!
// The results directory generated by the grid middleware starts with the grid 
// user name
$dir = dir(".");	
$dir->rewind();
	while ($file = $dir->read())
	{	
		...
		// Or
		// $contains=strpos($file, $server_user);
		// if (($contains !== false) && ($contains==0))
		// Or
		// if ((is_dir($file) && (is_file("$file/gramm-come.tar.gz"))
		{
...
function show_structure($message, $file)
{
	...
	echo "<tr>";
	echo "<td>$message</td>"; 
	echo "<td><a href=./$file>". basename($file, '.pdb') ."</a></td>";
	...
	echo "<td><a href=$pdb2png?url=$server$path/$file>PNG</a></td>";
	echo "</tr>";	
}
...
function show_results()
{
	...
	$file = 'receptor.pdb';
	if (file_exists($file))
		show_structure('Structure of your receptor molecule', $file);

	$file = 'ligand.pdb';
	if (file_exists($file))
		show_structure('Structure of your ligand molecule, $file);

	// walk through our working directory and look for R-L complexes.
	$dir->rewind();
	while ($file = $dir->read())
  	{
		// Regular expression to show the receptor-ligand files
		if ( ereg("^receptor-ligand+_[0-9]+.pdb$", $file))
			show_structure('Structure of receptor-ligand complex', $file);
	}
	$dir->close();
	
	$file = 'receptor-ligand.res');
	if ( file_exists($file) )
	{
		echo "<tr>";
