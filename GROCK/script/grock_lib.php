...
/* This should go with a read_options() function as well
function save_options()
{
    global $ck_auth, $ck_options;
    // try to cover our ass
    umask(077);

    // save options on a file to pass them on to GROCK
    $grockopt = fopen("options", "w+");
    foreach ($ck_auth as $idx => $value) fwrite($grockopt, "$idx=$value\n");
    fwrite($grockopt, "\n");
    foreach ($ck_options as $idx => $value) fwrite($grockopt, "$idx=$value\n");
    fclose($grockopt);
    
    // encrypt the file, this is ugly, needs more thought, and should
    // be done as well in processor.php, and reading the file should
    // be done decrypting it.
    // Perhaps it is time for the trick of sending all values in a single line
    // Plus, this only works on PHP5 (as fpc and fgc are not in PHP4)
    // Note: they are in PEAR package PHP_Compat (worth looking at)
    // NOTE: it also requires mcrypt support installed
    $key = $ck_session . $ck_
    file_put_contents(
        'options', 
        mcrypt_encrypt(
            MCRYPT_RIJNDAEL_256,
            implode($ck_auth),		// this may depend on array order
...
{
    log_error('taking a checkpoint because of $sig\n');
    exit;
    switch ($sig) {
        // save options back to a file
        // The following would require pertinent information to be
...
{   	
	...
	while ( (int)$status != (int)EODB ) {
    	// the need for the above castings mistifies me!!!
