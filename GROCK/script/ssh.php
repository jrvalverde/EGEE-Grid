 ...
 * @license	../c/lgpl.txt
 * @version	CVS: $Id: ssh.php,v 1.1.1.1 2005/09/07 13:15:49 root Exp $
...
*/  	
	    ...
	    "$this->ssh -x -T -C -S $this->mplex_socket $this->remote " .
	    		...
	    		"/bin/tar -cf - ". basename($remotepath) .")\" | ".
		...
		//  NOTE: not  quite right -- what if it is a wildcard?
		if ($debug_sexec) echo "-->Executing.s\n" .
		    "($this->ssh -x -T -C -S $this->mplex_socket $this->remote " .
		    "\"cat $remotepath\" | ".
		    " cat > $localpath) 2>&1\n";
	    	exec("($this->ssh -x -T -C -S $this->mplex_socket $this->remote " .
...
#    }
    ...
    function ssh_popen($command, $mode)
    {
    	...
    	if ($debug_sexec) echo "Grid::ssh_popen($command, $mode)\n";
	// go
	return popen("$this->ssh -x -T -S $this->mplex_socket $this->remote \"$command\"", $mode);
    }
