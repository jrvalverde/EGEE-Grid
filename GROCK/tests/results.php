...
/**
 * Retrieve the session identifier and the ligand name
 */ 
$session_id=$_GET["id"];
$session_ligand=$_GET["probe"];
...
$remote_wd="$gridUI_tmp_path/$session_id";

/**
 * Files to control script execution
 */
$process_control = "$local_wd/grock_status";
...
{  
    letal("GROCK results", "GROCK jobs haven't been submitted, or may be you have an invalid session identifier.");  
}
...
{   
   /**
    * We connect to the GRID to check each job sent. If the job is finished
    * we copy the output to the receptor local directory.
    */
   echo "<h1>Retrieving jobs output.</h1><br>";
   // REMEMBER... TO CHANGE!!!
   get_grid_results($local_wd, $remote_wd, $session_id, $app_dir, $grid_server, $grid_password, $grid_passphrase, $gridUI_tmp_path);
   ...
   if ($check==TRUE)
   {
	...
	echo "<h1>PROCESO FINALIZADO<br>";
    	/**
    	 * The signal to notice all jobs are done is the "jobs_output.txt" file
         */
