...
class Grid {
    ...
    var $session;   	/**< Internal: A unique identifier for this session */
    var $sx;	    	/**< Internal: The secure transfer communications line to use */
...
//	    fgets($this->std_out, 1024);
     ...
     *
     *	Note that once you call this, no more work will be performed on
     * the Grid on your behalf... BEWARE of race conditions!!!
     *
     *	As an example, suppose you have two simultaneous works, A and B:
     ...
     *
     *	This function submits a job to the Grid, optionally tagging it
     * as part of a specific session.
     *
     *	@param	job	The name of the job (same as the subdirectory it is in)
     *	@param	session Optional name of the session to which this job belongs
	...
	* local code above.
	...
	// check job-info.out exists: if not, then warn user
...
// wait for completion and copy locally as above and return.
	...
	fwrite($this->std_in, "edg-job-get-output " .
	    "--dir $this->work_dir/$session/$rjob/ " . 
	    $job_id[1] . "> job.out\n");
	sleep(5);   // XXX JR XXX should check stdout
	if ($debug)
