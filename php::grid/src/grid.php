...
class Grid {
     ...
     * not hang. Currently job submission using edg-job-submit/glite-job-submit 
     * may hang ccassinally. This is rare, maybe one out of 4-5 thousand jobs,
     ...
     *
     *	For the same reason, edg-job-submit/glite-job-submit has no way to know if the
     ...
     * from edg-job-submit/glite-job-submit point of view, most probably it is not a bug,
     ...
     *	grid-proxy-init/voms-proxy-init itself), but their duration may be fine tuned
     ...
     *
     *  @bug: XXX JR XXX grid-proxy-init/voms-proxy-init will use the time from the last
	...
	//  dismissing remote command output.
	    ...
	    ". /etc/bashrc ; " .
	    // grock_lib.php script
	    //"/opt/globus/bin/grid-proxy-init -pwstdin -valid 300:00", 
	    //"/opt/globus/bin/grid-proxy-init -pwstdin -valid $hours:$minutes", 
	    "/opt/edg/bin/voms-proxy-init -voms biomed -pwstdin -valid $hours:$minutes",
...
*/
     ...
     * This may happen if you have a load-balancing web server
     * cluster or if you want to redirect job management to a
     * new web page.
     *
	...
	if ($debug_grid)
	    ...
	    //. "/opt/glite/bin/glite-job-submit"
	...
	if ($status != 0) {
	    ...
	    exec("touch /tmp/debug/$remote_dir.status.txt");
	    exec("echo $this->work_dir/$sd/$remote_dir >> /tmp/debug/identifiers.txt");
    	    return FALSE;
    	}
	*/
	...
	if ($debug_grid)
		...
		"/opt/edg/bin/edg-job-status ". $job_id[1] .")";
	    	//"/opt/glite/bin/glite-job-status ". $job_id[1] .")";
	...
	if ($status != 0) {
	    ...
	    $status = $this->sx->ssh_exec(
		    ...
		    //"/opt/glite/bin/glite-job-output " .
...
*/
	...
	return $this->sx->ssh_exec(
	    ...
	    //"yes | /opt/glite/bin/glite-job-cancel ".
	    $job_id[1].
	    ")",
    	    $out);
    }
