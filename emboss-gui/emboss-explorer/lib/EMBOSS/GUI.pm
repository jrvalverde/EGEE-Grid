...
OutputSandbox = { "output.tgz", "out", "err" };
Requirements = RegExp("cnb.uam.es", other.GlueCEUniqueId);
END_JDL
	close JOB_JDL;
	...
	my $cpid = open3("<&NULL", ">&GRID_OUT", ">&GRID_ERR", 
			"/var/www/html/emboss/egee.sh");
	waitpid $cpid, 0;
	close NULL;
	close GRID_ERR;
	close GRID_OUT;
