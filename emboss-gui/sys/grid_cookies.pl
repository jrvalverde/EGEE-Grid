...
# grid_cookies - sample CGI script that uses a cookie to get/set 
# encrypted user ID data

### JR ### The logic needs to be worked out and cleaned up, and
### constants need to be extracted
...
my $key = "15024480201502448021"; # this is unsafe, but will have to do for now until I think of a better approach
...
# Check the cookie (it may exist but be invalid)

# note: a password and passphrase may contain blanks, but not
# the username which must be a single word (anything after a
# blank on it might be later mistaken as an extra parameter)
my @uwords;
if ($uinfo[0]) {
  @uwords = split('\b', $uinfo[0]);
}

# extract values if they exist or assign safe defaults otherwise
