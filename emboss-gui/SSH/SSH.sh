...
#
# To *test*, run as 
#	ssh -x -T localhost `pwd`/SSH.sh $user@host
#	enter-password
# this will run this script under NO TTY.
#
# To use: it must be called from a process with no control tty (such
# as a web server) and invoked directly:
#
#	./SSH.sh $user@host
#	password
#	command1
...
# This works to force allocating a TTY (when the program to run requires one)
##ssh -x -t -t $1
# This works to avoid trying to allocate a TTY when there is none (e.g.
